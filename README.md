...
- Startup ordering: healthchecks and `depends_on` conditions are important, but application-level retries are still required.
- Networking/debugging: service-name DNS is reliable, but diagnosing connection issues required validating ports, names, and runtime readiness.
- Volume permissions: host/container ownership mismatches can break initialization; explicit `chown` and directory bootstrap are critical.
- TLS setup: certificate generation and config validation (`nginx -t`) should run during startup to fail fast on misconfiguration.
